...
class UserService {
  async getOne(req) {
    ...
    let resObj = await userRepo.getOne(recordId);
    let result = {
      data: resObj,
  ...
  async getAll(req) {
    ...
    let resObj = await userRepo.getAll(filterExp);
    let result = {
      data: resObj,
  ...
  async getCount(req) {
    let filterExp = req.params.filter || '';
    let resObj = await userRepo.getCount(filterExp);
    let result = {
      data: resObj,
  ...
  async create(req) {
    ...
    let resObj = await userRepo.create(req.body);
    let result = {
      data: resObj,
  ...
  async update(req) {
    ...
    if(!filterExp || (filterExp && Object.keys(filterExp).length === 0)) {
      ...
      let resObj = await userRepo.update(filterExp, requestedDataToUpdate);
  ...
  async delete(req) {
    ...
    if(!filterExp || (filterExp && Object.keys(filterExp).length === 0)) {
      ...
      let resObj = await userRepo.delete(filterExp);
      let result = {
